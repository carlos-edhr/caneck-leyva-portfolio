import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { Facebook, Instagram, Youtube } from "lucide-react";
import Link from "next/link";

const DashboardFooter = () => {
  return (
    <footer className=" w-full  bg-stone-950 text-gray-100 shadow-lg p-[15px]">
      <div className="w-full flex  flex-col  items-center justify-center m-auto">
        <div className="w-full h-full flex flex-row items-center justify-around flex-wrap">
          <div className="min-w-[200px] h-auto flex flex-col items-center justify-start">
            <div className="font-bold text-[16px]">Comunidad</div>
            <Separator className="w-full max-w-xs h-[1px] bg-slate-700 m-1" />
            <Link
              target="_blank"
              href="https://www.youtube.com/@ifnastro/videos"
            >
              <p className="flex flex-row items-center my-[4px] cursor-pointer">
                <Youtube />

                <span className="text-[15px] ml-[6px]">Youtube</span>
              </p>
            </Link>
            <Link target="_blank" href="https://www.instagram.com/ifnastro/">
              <p className="flex flex-row items-center my-[4px] cursor-pointer">
                <Instagram />
                <span className="text-[15px] ml-[6px]">Instagram</span>
              </p>
            </Link>

            <Link target="_blank" href="https://www.facebook.com/ifnastro">
              <p className="flex flex-row items-center my-[4px] cursor-pointer">
                <Facebook />
                <span className="text-[15px] ml-[6px]">Facebook</span>
              </p>
            </Link>
          </div>

          <div className="min-w-[200px] h-auto flex flex-col items-center justify-start">
            <div className="font-bold text-[16px]">Acerca de nosotros</div>
            <Separator className="w-full max-w-xs h-[1px] bg-slate-700 m-1" />
            <Link href="/" target="_blank">
              <p className="flex flex-row items-center my-[4px] cursor-pointer">
                <span className="text-[15px] ml-[6px]">
                  Regístrate al Congreso 2025
                </span>
              </p>
            </Link>
            <Link href="https://www.ifntijuana.com/" target="_blank">
              <p className="flex flex-row items-center my-[4px] cursor-pointer">
                <span className="text-[15px] ml-[6px]">
                  Instituto de Fotografía del Noroeste
                </span>
              </p>
            </Link>
            <Link href="https://www.caneckleyva.com/" target="_blank">
              <p className="flex flex-row items-center my-[4px] cursor-pointer">
                <span className="text-[15px] ml-[6px]">Caneck Leyva</span>
              </p>
            </Link>
            {/* <p className="flex flex-row items-center my-[4px] cursor-pointer">
              <span className="text-[15px] ml-[6px]">Nuestra misión</span>
            </p> */}
            <p className="flex flex-row items-center my-[4px] cursor-pointer">
              <span className="text-[15px] ml-[6px]">
                congreso@leyvafotografia.com
              </span>
            </p>
          </div>
        </div>

        <div className="z-40 mb-[20px] text-[15px] text-center">
          &copy; Congreso Internacional de Astrofotografía 2025.
        </div>
        <div className="flex z-40 mb-[20px] text-[15px] text-center justify-center align-center">
          <p className="flex align-middle justify-center">
            Developed by &nbsp;
            <Link target="_blank" href="https://www.carlos-ehr.com">
              <span className="text-primaryLanding">carlos-ehr</span>
            </Link>
          </p>
        </div>
      </div>
    </footer>
  );
};

export default DashboardFooter;
